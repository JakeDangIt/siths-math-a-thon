export const useAnswersStore = defineStore("answers", () => {
  ...
  async function submitAnswers(week, answers) {
    const { data, error } = await supabase.from("submitted-answers").select("*").eq("uid", user.value.id).eq("week", week);
    ...
    if (data.length > 0) {
      ...
      const { error: updateError } = await supabase
        ...
        .update({ created_at: new Date().toISOString(), 'week': String(week), answers: answers })
        .eq("uid", user.value.id)
        .eq("week", week);
