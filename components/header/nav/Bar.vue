...
<template>
  <header class="flex h-12 items-center bg-theme-blue px-4 py-1 dark:bg-theme-dark-blue">
    <!-- left + home -->
    <div class="flex h-full flex-1 items-center">
      <nuxt-link to="/" class="flex h-full w-fit items-center" draggable="false" aria-label="Home">
    ...
    <nav class="flex flex-1 justify-center gap-2">
      <HeaderNavLink v-for="route in routes.slice(1, 3)" :route-path="route.routePath" :route-name="route.routeName"
        variant="link" :textColor="'black'" class="flex justify-center text-lg hover:bg-gray-500 hover:bg-opacity-20"
        :class="{ 'font-bold': currentRoutePath == route.routePath }" />

      <DropdownMenu>
        <DropdownMenuTrigger class="flex flex-1 items-center justify-center gap-1 text-lg">
          Resources
        ...
        <DropdownMenuContent>
          <DropdownMenuItem v-for="route in routes.slice(3, 6)">
            <HeaderNavLink :route-path="route.routePath" :route-name="route.routeName" variant="link"
              class="w-full text-lg" :textColor="'white'"
              :class="{ 'font-bold': currentRoutePath == route.routePath }" />
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
